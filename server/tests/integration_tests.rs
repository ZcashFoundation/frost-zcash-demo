use std::str::FromStr;
use std::{collections::BTreeMap, io::BufReader, time::Duration};
...
#[tokio::test]
async fn test_coordinator() -> Result<(), Box<dyn std::error::Error>> {
    // Create key shares
    let mut rng = thread_rng();
    let (shares, pubkeys) =
        frost::keys::generate_with_dealer(3, 2, frost::keys::IdentifierList::Default, &mut rng)
    ...
    std::fs::write("public-key-package.json", serde_json::to_vec(&pubkeys)?)?;
    ...
    let message1 = "Hello, world!".as_bytes();
    std::fs::write("message.raw", message1)?;
    let aux_msg = "Aux msg".as_bytes();
    let messages = [message1];
    let mut reader = BufReader::new(std::io::empty());
    let logger = Output::new();
    let coordinator_args = coordinator::args::Args {
        cli: false,
        http: true,
        ip: "127.0.0.1".to_string(),
        num_signers: 2,
        public_key_package: "public-key-package.json".to_string(),
        message: "message.raw".to_string(),
        signature: "signature.raw".to_string(),
        #[cfg(feature = "redpallas")]
        randomizer: "".to_string(),
        port: 2744,
    };
    // Spawn coordinator
    let mut cloned_logger = logger.clone();
    tokio::task::spawn_local(async move {
        coordinator::cli::cli(&coordinator_args, &mut reader, &mut cloned_logger)
            .await
            .unwrap();
    });

    eprint!("Waiting for session ID...");
    let re =
        regex::bytes::Regex::new(r"[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}")
            .unwrap();
    let session_id = loop {
        let log = logger.bytes();
        if let Some(m) = re.find(&log) {
            break Uuid::from_str(&String::from_utf8(m.as_bytes().to_vec()).unwrap())?;
        }
        tokio::time::sleep(Duration::from_secs(2)).await;
        eprint!(".");
    };
